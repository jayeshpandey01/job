...
const ViewApplications = () => {
  ...
  useEffect(() => {
    if (!highlightId || loading || applicants.length === 0) return;
    const el = document.getElementById(`application-${highlightId}`);
    el?.scrollIntoView({ behavior: "smooth", block: "center" });
  }, [highlightId, loading, applicants]);
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      default:
        return (
          <div className="flex items-center gap-1 bg-brand-orange/10 text-brand-orange px-3 py-2 rounded-full text-sm font-medium">
            <ChevronDown size={14} />
            <span>Pending</span>
          </div>
        );
  ...
  return (
    <div className="bg-jl-bg min-h-screen py-8">
      <div className="container mx-auto px-4">
        <motion.div
          ...
          className="mb-8"
        >
          <h1 className="text-2xl md:text-3xl font-bold text-brand-navy mb-2">Application Management</h1>
          ...
          <div className="p-5 border-b border-gray-100 flex flex-wrap gap-4 items-center justify-between">
            <div className="relative flex-1 min-w-[250px]">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2.5 w-full rounded-lg border border-gray-200 focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange transition-colors"
                value={searchTerm}
                ...
                {filteredApplicants.length === 0 ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-16">
                        ...
                        {(searchTerm || filterStatus !== "all") && (
                          <button 
                            ...
                            className="mt-2 bg-brand-orange/10 text-brand-orange px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-orange/20 transition-colors"
                          >
                            Clear filters
                          </button>
                        )}
                  ...
                  filteredApplicants.map((applicant, index) => (
                    <motion.tr 
                      ...
                      id={applicant._id ? `application-${applicant._id}` : undefined}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      transition={{ duration: 0.3, delay: index * 0.05 }}
                      className={`border-b border-gray-100 hover:bg-brand-orange/5 transition-colors ${
                        applicant._id === highlightId ? "bg-brand-orange/10 ring-2 ring-inset ring-brand-orange/40" : ""
                      }`}
                    >
                      ...
                      <td className="py-4 px-5">
                        <a
                          ...
                          className="inline-flex items-center gap-2 bg-brand-orange/10 text-brand-orange px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-orange/20 transition-colors"
                        >
                          <FileText size={16} />
                          View Resume
                        </a>
                        ...
                        {applicant.status === "pending" ? (
                          <div className="relative">
                            <button 
                              ...
                              className="bg-brand-orange/10 text-brand-orange px-3 py-2 rounded-lg text-sm font-medium hover:bg-brand-orange/20 transition-colors flex items-center gap-1"
                            >
                              <span>Pending</span>
